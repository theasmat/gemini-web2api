...
def get_auth_details() -> dict:
    """Return diagnostic details of active authentication state."""
    ...
    cookie_names = []
    if cookie_str:
        cookie_names = [p.split("=")[0].strip() for p in cookie_str.split(";") if "=" in p]

    return {
        "authenticated": has_cookie and has_sapisid,
        ...
        try:
            if self.path in ("/dash", "/dash/", "/dashboard"):
                try:
                    ...
                    self.send_html("<h1>Dashboard</h1><p>Dashboard module available via modular package.</p>")
                return
...
def main():
    ...
    parser.add_argument("--login", action="store_true", help="Launch automated browser login to capture credentials")
    parser.add_argument("--version", action="version", version=f"gemini-web2api {__version__}")
    args = parser.parse_args()

    if args.login:
        try:
            from gemini_web2api.login import run_login_flow
            print("[*] Launching browser login automation...")
            run_login_flow()
        except ImportError:
            print("[!] Login module not available.")
    ...
    print(f"  Cookie:    {'yes (' + str(CONFIG['cookie_file']) + ')' if CONFIG.get('cookie_file') else 'none (anonymous)'}")
