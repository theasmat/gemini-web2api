...
const syncServerButton = document.getElementById("sync-server");
...
syncServerButton.addEventListener("click", async () => {
  syncServerButton.disabled = true;
  setStatus("Extracting auth data and syncing to local server…");
  ...
  try {
    ...
    setStatus(
      ...
      `gemini_bl: ${info.pageMetadata.geminiBl || "default"}\n\n` +
      `The local server has updated its active credentials live without restart.`,
      "ok"
    );
  } catch (error) {
    setStatus(`Sync failed: ${error?.message || String(error)}`, "warn");
  } finally {
    syncServerButton.disabled = false;
  }
});
